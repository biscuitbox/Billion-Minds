# Billion Minds

> 10억 개의 픽셀로 던지는 질문 — *과연 인류의 마음은 하나로 모일 수 있는가?*

## 프로젝트 소개

**Billion Minds**는 10억 개(1,000,000,000)의 픽셀로 이루어진 거대한 캔버스 위에
매 시간마다 무작위 색깔을 새로 칠하는 웹 기반 수학적 확률 실험입니다.

매 시간 새로 그려진 그림은 **최초의 그림(Genesis Frame)** 과 픽셀 단위로 비교되며,
두 그림 사이의 **유사도(Similarity)** 가 백분율로 기록됩니다.
시간이 흐르며 누적되는 유사도 변화율은 꺾은선 그래프로 시각화되어,
"무한한 무작위성 속에서 동일한 결과는 얼마나 자주 나타나는가?"를 관측하게 합니다.

이 프로젝트의 본질적인 질문은 단순합니다.

> **인류의 마음, 혹은 목적은 과연 일치될 수 있는가?**

10억 개의 픽셀이 우연히 같은 색을 칠할 확률은,
80억 명의 인류가 같은 마음을 품을 확률에 대한 하나의 비유입니다.

---

## 핵심 컨셉

- **Genesis Frame** — 사이트가 처음 생성한 10억 픽셀의 무작위 그림. 모든 비교의 기준점.
- **Hourly Refresh** — 매 정시(00분)마다 10억 픽셀을 무작위 색으로 새로 칠합니다.
- **Similarity Score** — 매 갱신마다 Genesis Frame과 픽셀별로 비교하여 일치율(%)을 산출합니다.
- **Convergence Graph** — 누적된 유사도를 꺾은선 그래프로 표시하여 변화율을 추적합니다.

---

## 수학적 배경

각 픽셀이 24bit 색상(약 1,677만 색)을 무작위로 가질 때,
임의의 두 픽셀이 정확히 같은 색일 확률은 약 $\frac{1}{2^{24}} \approx 5.96 \times 10^{-8}$ 입니다.

10억 픽셀 전체의 유사도(완전 일치 픽셀 비율)의 기댓값은:

$$
E[\text{Similarity}] = \frac{N_{\text{match}}}{N_{\text{total}}} \approx \frac{10^9}{2^{24}} \div 10^9 \approx 5.96 \times 10^{-8}
$$

즉, **평균적으로 약 0.0000059%** 의 픽셀만이 우연히 일치합니다.

이 미세한 확률을 시간에 따라 관측함으로써, "수학적 우연"과 "의미 있는 수렴" 사이의 거리를 시각화합니다.

---

## 기능 명세

### 1. 픽셀 캔버스 렌더링
- 10억 픽셀을 사용자 화면에 효율적으로 표시 (예: 31,623 × 31,623 격자 또는 타일링).
- WebGL / Canvas2D / 서버 사이드 이미지 생성 등 렌더링 전략 비교 검토.
- 사용자 확대/축소(zoom & pan) 인터랙션 지원.

### 2. 매시간 자동 갱신
- 정시 cron 기반 트리거.
- 시드 기반 무작위 색 생성으로 결과 재현 가능성 확보.
- 갱신 이력은 영구 저장 (DB 또는 오브젝트 스토리지).

### 3. 유사도 계산
- Genesis Frame과 현재 Frame을 픽셀 단위 비교.
- "정확 일치(exact match)" 외에 색 거리(Euclidean, ΔE 등) 기반 근사 유사도 옵션도 제공 검토.
- 결과는 시각(timestamp), 유사도(%), 일치 픽셀 수와 함께 기록.

### 4. 변화율 그래프
- X축: 시간 (시 단위).
- Y축: 유사도 백분율.
- 꺾은선 그래프로 누적 추세 표시.
- 이론적 기댓값 보조선과 비교 가능.

### 5. 히스토리 아카이브
- 과거 Frame을 시드로 재생성하여 열람 가능.
- 특정 시점의 픽셀 좌표 단위 검색 기능.

---

## 기술 스택 (예정)

| 영역 | 후보 |
|---|---|
| 백엔드 | Python (FastAPI) |
| 픽셀 생성 | NumPy / Pillow / 시드 기반 PRNG |
| 저장소 | PostgreSQL (메타데이터) + S3 호환 스토리지 (이미지/시드) |
| 스케줄러 | APScheduler / cron |
| 프런트엔드 | React / WebGL / D3.js (그래프) |
| 배포 | Docker, Cloudflare 또는 AWS |

> 위 스택은 초기 구상이며 프로젝트 진행에 따라 변경될 수 있습니다.

---

## 로드맵

- [ ] **Phase 0** — 프로젝트 셋업, 시드 기반 픽셀 생성기 프로토타입
- [ ] **Phase 1** — Genesis Frame 생성 및 영구 저장
- [ ] **Phase 2** — 매시간 갱신 스케줄러 + 유사도 계산 파이프라인
- [ ] **Phase 3** — 캔버스 렌더링 웹 UI
- [ ] **Phase 4** — 유사도 변화 그래프 대시보드
- [ ] **Phase 5** — 히스토리 아카이브 및 공개 API

---

## 철학적 메모

> *"무한한 시간 속에서, 우연히도 같은 색을 가진 두 점은 서로를 알아볼 수 있을까?"*

이 사이트의 숫자는 차갑지만, 그 너머에 있는 질문은 따뜻합니다.
서로 다른 80억의 마음이 단 한 순간이라도 같은 색을 띨 수 있다면,
그것이 0.0000001%의 우연이라 할지라도 — 우리는 그것을 **의미**라고 부를 수 있을 것입니다.

---

## 라이선스

추후 결정 (MIT 검토 중).
